...
#        print ("pageData()打开数据库成功")
    ...
    def lineDisplay(self,l): #在edit中显示browser 1-5行的内容
        print("g=",NextOffice.tt.main.g)
        p=NextOffice.tt.main.g*5-4 #获得当前页面的第1行
        data=self.database()
        if l==1:
...
#        curs.execute("UPDATE customer SET customer='name' WHERE customerID=20180001;")

        curs.execute("UPDATE customer set customer=?, nickname1=?, nickname2=?,nickname3=?,contact=?,remark=? WHERE customerID=?;",content)

#        curs.execute("UPDATE customer set customer='name', nickname1='nick1', nickname2='nick2',nickname3='nick3',contact='contact',remark='remark' WHERE customerID=20180001;")
