...
#         self.dateEdit_pay.setDate(str(data[p-1][11]))
    ...
    def saveToSqlite(self):
        ...
        bit=1
        content=[shipDate,customer,sales,production,model,macID,unitPrice,number,paid,paidRecord,payRemind,shipTo,remark,bit,id]
...
### 以下开始连接数据库
                conn = sqlite3.connect('nextai.db')
                curs = conn.cursor()  #创建游标
                curs.execute("UPDATE shipment set shipDate=?, customer=?,sales=?,production=?,model=?,macID=?,unitPrice=?,number=?,paid=?,paidRecord=?,payRemind=?,shipTo=?,remark=?,bit=? WHERE shipmentID=?;",content)
                curs.close()  #关闭游标
                conn.commit() #保存数据库
                conn.close() #关闭数据库连接
                self.close()
